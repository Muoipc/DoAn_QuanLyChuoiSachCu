...
public class BookController {
    ...
    public String bookDetail(@PathVariable("id") Long id, Model model) {
        ...
        // 6. Đưa dữ liệu vào Model cho Thymeleaf render
    ...
    public String bookDetailBySlug(@PathVariable("slug") String slug, Model model) {
        Optional<Book> bookOpt = bookRepository.findDetailBySlug(slug);
        if (bookOpt.isPresent()) {
            return bookDetail(bookOpt.get().getId(), model);
        }
        return "redirect:/";
    }
}
